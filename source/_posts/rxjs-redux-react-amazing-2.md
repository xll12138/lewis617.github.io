...
- 自动补全（他先是演示了使用普通JavaScript的写法，代码特别长，过程特别繁琐，然后演示了使用Epic的写法，只用了debounce、switchMap和map三个操作符，代码特别短，特别声明式，特别易读）
- 双向多重的WebSockets（他先是说这个例子绝对够硬，够真实， 然后还是先演示用普通JavaScript代码的写法，代码特别长，特别容易引入bug， 他会可怜写这种代码的人，如果他自己写了这种代码，他一定也会可怜自己。然后他演示了Epic的写法，功能一样不少，但是代码特别少，特别声明式）
...
但是在你尝试使用redux-observable之前，**你应该提前学习Redux和RxJS**。当然，这应该是业余时间做的事，如果你特别想挑战自己。我不是打击大家的积极性，只是不想让在座的各位产生一个错误观念，就是无论我的app有多小多简单，我都应该使用redux-observable。 但是，如果你觉得我今天描述的问题，也正好是你的问题，那么你一定愿意学习RxJS和Redux。 

另外，RxJS拥有非常陡峭的学习曲线。 最近有个新流行词叫响应式编程（Reactive Programing ），RxJS做的就是这个。响应式编程是个令人发狂的编程范式，不过你可以这么理解它：就像是你先安装好管道，但还没有水流过这些管道，等到将来某个人发起了一个action，然后潺潺流水就来了（他之所以用流水这个比喻，主要是为了突出响应式编程，自动响应和变化传播的特性，如果你还不能理解，可以参考[维基百科](https://zh.wikipedia.org/wiki/%E5%93%8D%E5%BA%94%E5%BC%8F%E7%BC%96%E7%A8%8B)）。响应式编程是一种完全不同的编程风格。
